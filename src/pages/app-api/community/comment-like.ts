...
import { toggleCommunityCommentLike } from '../../../lib/pocketbase';
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const result = await toggleCommunityCommentLike(userToken, {
		...
		return new Response(
			JSON.stringify({
				message: parseErrorMessage(error) || 'Impossible de mettre à jour ce like.',
			}),
			{
