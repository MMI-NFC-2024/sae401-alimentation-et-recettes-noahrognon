...
import { getUserAppointments } from '../../lib/pocketbase';
...
export const GET: APIRoute = async ({ request }) => {
	try {
		...
		const result = await getUserAppointments(userToken);
		...
		return new Response(
			JSON.stringify({
				message: parseErrorMessage(error) || 'Impossible de charger vos rendez-vous.',
			}),
			{
