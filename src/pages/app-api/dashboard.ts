...
import { getUserDashboard } from '../../lib/pocketbase';
...
export const GET: APIRoute = async ({ request }) => {
	try {
		...
		const result = await getUserDashboard(userToken);
		...
		return new Response(
			JSON.stringify({
				message: parseErrorMessage(error) || 'Impossible de charger le tableau de bord.',
			}),
			{
