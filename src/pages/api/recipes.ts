...
import { getRecipesCatalog } from '../../lib/pocketbase';
...
export const GET: APIRoute = async ({ request }) => {
	try {
		...
		const result = await getRecipesCatalog(userToken);
		...
		return new Response(
			JSON.stringify({
				message: parseErrorMessage(error) || 'Impossible de charger les recettes.',
			}),
			{
