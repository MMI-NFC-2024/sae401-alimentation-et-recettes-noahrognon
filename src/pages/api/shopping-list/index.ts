...
export const GET: APIRoute = async ({ request, url }) => {
	try {
		...
		const result = await getShoppingList(userToken, date);
		return new Response(JSON.stringify(result), {
			status: 200,
			headers: { 'Content-Type': 'application/json' },
		});
	} catch (error) {
		return new Response(JSON.stringify({ message: parseErrorMessage(error) || 'Impossible de charger la liste de courses.' }), {
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const result =
			action === 'add-manual'
				? await addManualShoppingListItem(userToken, {
				...
				: await toggleShoppingListItem(userToken, {
						weekStart: String(body.weekStart),
		...
		return new Response(JSON.stringify({ message: parseErrorMessage(error) || 'Impossible de mettre à jour la liste de courses.' }), {
...
export const DELETE: APIRoute = async ({ request }) => {
	try {
		...
		const body = await request.json();
		const result = await removeShoppingListItem(userToken, {
			weekStart: String(body.weekStart),
			itemKey: String(body.itemKey),
		});
		...
		return new Response(JSON.stringify({ message: parseErrorMessage(error) || 'Impossible de supprimer cet article.' }), {
			status: isUnauthorizedError(error) ? 401 : 400,
			headers: { 'Content-Type': 'application/json' },
		});
	}
