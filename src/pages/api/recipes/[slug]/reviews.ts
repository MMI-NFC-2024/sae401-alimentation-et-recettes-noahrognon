...
import { addRecipeReview } from '../../../../lib/pocketbase';
...
export const POST: APIRoute = async ({ request, params }) => {
	try {
		...
		const result = await addRecipeReview(userToken, slug, {
		...
		return new Response(
			JSON.stringify({
				message: parseErrorMessage(error) || 'Impossible d’ajouter l’avis.',
			}),
			{
